...
$categorias = new CCategorias();
	...
	$totalRegistro=$categorias->contarCategoriasIna();	
	...
	$datosPag=$categorias->consultarCategoriasIna($inicioPag,$numRegistro);			
    $estado=1;
   
    //VERIFICA QUE LA VARIABLE ne TENGA VALOR PARA ELIMINAR AL USUARIO
    if (isset($_GET['ne'])){
       
       	$datosDel=$categorias->activarCategorias($_GET['ne'], $_GET['cat']);
           print_r($datosDel);	
                    ...
                    <?php
                      ...
                      {	          
                    	for($rr=0;$rr<count($datosPag);$rr++){	
                    		echo "<tr>";
                    		echo "<td bg-primary><a href='?ruta=activar-categoria&ne=".$datosPag[$rr]["Clave"]."&cat=".$datosPag[$rr]["Categoria"]."' class='btn btn-sys btn-options' title='Activar' ><i class='fas fa-sync-alt'></i></a>";
                	  ...
                	  {		
                		...
                		{		
                			echo " <a href='?ruta=activar-categoria&pagina=$i'>$i</a> ";
                		}
                	  }
                	  echo "</center>";
