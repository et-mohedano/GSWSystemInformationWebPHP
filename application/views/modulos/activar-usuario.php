...
$usuarios = new cUsuario();	
	...
	$totalRegistro=$usuarios->contarUsuariosIna();	
	...
	$datosPag=$usuarios->consultarUsuariosIna($inicioPag,$numRegistro);			
    $estado=1;
   
    //VERIFICA QUE LA VARIABLE ne TENGA VALOR PARA ELIMINAR AL USUARIO
    if (isset($_GET['ne'])){
       
       	$datosDel=$usuarios->actUsuario($_GET['ne']);	
                    ...
                    <?php
                      ...
                      {	          
                    	for($rr=0;$rr<count($datosPag);$rr++){	
                    		echo "<tr>";
                    		echo "<td bg-primary><a href='?ruta=activar-usuario&ne=".$datosPag[$rr]["CLAVE"]."' class='btn btn-sys btn-options' title='Activar' ><i class='fas fa-sync-alt'></i></a>";
                	  ...
                	  {		
                		...
                		{		
                			echo " <a href='?ruta=activar-usuario&pagina=$i'>$i</a> ";
                		}
                	  }
                	  echo "</center>";
