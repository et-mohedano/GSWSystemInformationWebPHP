...
$proveedores = new cProveedor();
	...
	$totalRegistro=$proveedores->contarProveedoresIna();	
	...
	$datosPag=$proveedores->consultarProveedoresIna($inicioPag,$numRegistro);			
    $estado=1;
   
    //VERIFICA QUE LA VARIABLE ne TENGA VALOR PARA ELIMINAR AL USUARIO
    if (isset($_GET['ne'])){
       
       	$datosDel=$proveedores->activarProveedores($_GET['ne']);	
                    ...
                    <?php
                      ...
                      {	          
                    	for($rr=0;$rr<count($datosPag);$rr++){	
                    		echo "<tr>";
                    		echo "<td bg-primary><a href='?ruta=activar-proveedores&ne=".$datosPag[$rr]["CLAVE"]."' class='btn btn-sys' title='Activar' ><i class='fas fa-sync-alt'></i></a></td>";
                	  ...
                	  {		
                		...
                		{		
                			echo " <a href='?ruta=activar-proveedor&pagina=$i'>$i</a> ";
                		}
                	  }
                	  echo "</center>";
