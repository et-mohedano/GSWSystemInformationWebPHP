...
<?php
    ...
    $datosPag=$productos->consultarProductosAlerta();
    ...
    for($reg = 0; $reg < count($datosPag); $reg++){
        ...
        if($fecha_prod <= $fecha_actual_2 && $fecha_prod >= $fecha_actual){
            ...
            echo '
            <div class="alert alert-danger alert-dismissible fade show" role="alert">
              <strong>¡Producto caducado! '.$datosPag[$reg]['NOMBRE'].' '.$datosPag[$reg]['PRESENTACION']. ' => ' .$datosPag[$reg]['CADUCIDAD'].'</strong><br><center><a href="?ruta=productos&ne='.$datosPag[$reg]["CLAVE"].'" class="" title="Eliminar" > Pulsa aquí para eliminarlo</a></center>
              <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
            </div>';
        }
        if($datosPag[$reg]['CANTIDAD'] <= 3 && $datosPag[$reg]['CANTIDAD'] > 0){
            ...
            <div class="alert alert-dark alert-dismissible fade show" role="alert">
              ...
              </strong><br><center><a href="?ruta=modificar-producto&cve='.$datosPag[$reg]["CLAVE"].'" class="" title="Eliminar" >Pulsa aquí para actualizarlo</a></center><button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
            </div>';
        }
    }
?>
