...
$productos = new cProducto();
	...
	$totalRegistro=$productos->contarProductos();
	...
	$datosPag=$productos->consultarProductos($inicioPag,$numRegistro);			
    $estado=1;
   
    //VERIFICA QUE LA VARIABLE ne TENGA VALOR PARA ELIMINAR AL USUARIO
    if (isset($_GET['ne'])){
       
       	$datosDel=$productos->eliminarProducto($_GET['ne']);	
       	if((int)$datosDel[0]["ID"]!=0)
       	{
       	    echo '<script language="javascript">alert("Producto eliminado correctamente")</script>';
       	    echo "<script language='javascript'>document.location.href = '?ruta=productos';</script>";
       	}
    }
              ...
              <form id="frmConexion" name="frmConexion" method="POST">
                    ...
                    <?php
                      ...
                      {	          
                    	for($rr=0;$rr<count($datosPag);$rr++){	
                    		echo "<tr>";
                    		echo "<td bg-primary><a href='?ruta=modificar-producto&cve=".$datosPag[$rr]["CLAVE"]."' class='btn btn-sys btn-options' title='Editar' ><i class='fa fa-pencil-square-o'></i></a>
                                <br/><a href='?ruta=productos&ne=".$datosPag[$rr]["CLAVE"]."' class='btn btn-sys' title='Eliminar' ><i class='fa fa-times-circle'></i></a></td>";
                    	   ...
                    	   // echo "<td><font class='Etiquetas2'>".$datosPag[$rr]["PRECIO_ADQUISICION"]."</td>";
                            echo "<td align='middle'><font class='Etiquetas2'><img src='assests/images/".$datosPag[$rr]["FOTO"]."' class='img-fluid logo-sys'/></td>";
                	  ...
                	  {		
                		...
                		{		
                			echo " <a href='?ruta=productos&pagina=$i'>$i</a> ";
                		}
                	  }
                	  echo "</center>";
                ...
                <hr>
                  <div class="text-center">
                      <a href='?ruta=registrar-producto' class="btn btn-sys">Registrar Productos&nbsp;</a>
                      <a href='?ruta=activar-producto' class="btn btn-sys">Activar Productos&nbsp;</a>
