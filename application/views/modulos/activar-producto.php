...
$productos = new cProducto();
	...
	$totalRegistro=$productos->contarProductosIna();	
	...
	$datosPag=$productos->consultarProductosIna($inicioPag,$numRegistro);			
    $estado=1;
   
    //VERIFICA QUE LA VARIABLE ne TENGA VALOR PARA ELIMINAR AL USUARIO
    if (isset($_GET['ne'])){
       
       	$datosDel=$productos->actProducto($_GET['ne']);	
                    ...
                    <?php
                      ...
                      {	          
                    	for($rr=0;$rr<count($datosPag);$rr++){	
                    		echo "<tr>";
                    		echo "<td bg-primary><a href='?ruta=activar-producto&ne=".$datosPag[$rr]["CLAVE"]."' class='btn btn-sys btn-options' title='Activar' ><i class='fas fa-sync-alt'></i></a>";
                	  ...
                	  {		
                		...
                		{		
                			echo " <a href='?ruta=activar-producto&pagina=$i'>$i</a> ";
                		}
                	  }
                	  echo "</center>";
