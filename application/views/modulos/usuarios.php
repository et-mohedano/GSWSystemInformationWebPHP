...
$numRegistro=3;
	...
	$totalRegistro=$usuarios->contarUsuarios();	
	...
	$datosPag=$usuarios->consultarUsuarios($inicioPag,$numRegistro);
    $estado=1;
   
    // //VERIFICA QUE LA VARIABLE ne TENGA VALOR PARA ELIMINAR AL USUARIO
    if (isset($_GET['ne'])){
       
       	$datosDel=$usuarios->eliminarUsuario($_GET['ne']);	
       	if((int)$datosDel[0]["ID"]!=0)
       	{
       	    echo '<script language="javascript">alert("Usuario eliminado correctamente")</script>';
       	    echo "<script language='javascript'>document.location.href = '?ruta=usuarios';</script>";
       	}
    }
              ...
              <form id="frmConexion" name="frmConexion" method="POST">
                    ...
                    <?php
                      ...
                      {	          
                    	for($rr=0;$rr<count($datosPag);$rr++){	
                    		echo "<tr>";
                    		echo "<td bg-primary><a href='?ruta=modificar-usuario&cve=".$datosPag[$rr]["CLAVE"]."' class='btn btn-sys btn-options' title='Editar' ><i class='fa fa-pencil-square-o'></i></a>
                                <br/><a href='?ruta=usuarios&ne=".$datosPag[$rr]["CLAVE"]."' class='btn btn-sys' title='Eliminar' ><i class='fa fa-times-circle'></i></a></td>";
                	  ...
                	  {		
                		...
                		{		
                			echo " <a href='?ruta=usuarios&pagina=$i'>$i</a> ";
                		}
                	  }
                	  echo "</center>";
                ...
                <hr>
                  <div class="text-center">
                      <a href='?ruta=registrar-usuario' class="btn btn-sys">Registrar Usuario&nbsp;</a>
                      <a href='?ruta=activar-usuario' class="btn btn-sys">Activar Usuario&nbsp;</a>
